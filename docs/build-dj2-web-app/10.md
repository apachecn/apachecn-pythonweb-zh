...
在本章中，我们将开始构建 MailApe，这是一个邮件列表管理器，允许用户启动邮件列表，注册邮件列表，然后向用户发送消息。订阅者必须确认他们对邮件列表的订阅，并且能够取消订阅。这将帮助我们确保 MailApe 不会被用于向用户发送垃圾邮件。

在本章中，我们将构建 Mail Ape 的核心 Django 功能：

*   我们将构建描述 MailApe 的模型，包括`MailingList`和`Subscriber`
