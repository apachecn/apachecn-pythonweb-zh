...
现在，让我们继续安装Flask。其所有依赖项将自动安装；您不需要手动安装依赖项。
...
我们已经准备好玩Flask了。我们将创建一个最小的 Flask 应用来演示 Flask 应用工作流。

# 最小Flask应用

让我们看看最小Flask应用是什么样子的：
...
就这样，我们完成了最小Flask应用。使用 Flask 配置和创建微服务非常简单。
...
现在，Flask 应用可通过先前生成的 API 获得。让我们测试一下，看看 Hello World！Flask应用的响应。您可以在浏览器中点击 URL，如以下屏幕截图所示：
...
Flask 是一个微框架，但它具有可扩展性，您可以根据需要添加更多功能。要开发 Todo 应用，我们可能需要一些基本功能，例如数据持久性和用户身份验证机制。因此，在处理Flask应用时，我们将使用一些Flask扩展。

Flask 注册表提供了许多扩展，它们是独立的包，您可以使用 Flask 应用实例轻松地配置它们。您可以在以下位置查看Flask扩展的完整列表：[http://flask.pocoo.org/extensions/](http://flask.pocoo.org/extensions/) 。

我们将使用以下Flask和Flask扩展包：
...
*   `static`：将静态内容放在`static`目录下是Flask的标准做法。因此，我们将对所有必需的静态内容使用此目录。
*   `templates`：Flask 内置了对 Jinja2 模板引擎的支持，并根据模块名称遵循模板文件的标准布局。稍后，当我们实际使用模板时，我们将看到对此的详细描述。
*   `todo`：这是一个独立的Flask模块或包，具有基本的待办事项功能。
...
SQLAlchemy 最著名的是它的**对象关系映射器**（**ORM**），这是一个可选组件，提供数据映射器模式，其中类可以以开放的、多种方式映射到数据库，允许对象模型和数据库模式从一开始就以完全解耦的方式开发。我们在这里使用`Flask-SQLAlchemy`扩展，它扩展了 SQLAlchemy 的支持。`Flask-SQLAlchemy`增强了可能需要与Flask应用集成的功能。
