...
为了让读者更容易理解接下来的章节，我们将使用许多**烧瓶扩展**（扩展烧瓶功能的包）中的第一个，名为**烧瓶脚本**。Flask 脚本允许程序员创建在 Flask 的**应用程序上下文**中起作用的命令，即 Flask 中允许修改`Flask`对象的状态。Flask 脚本附带了一些默认命令，用于在应用程序上下文中运行服务器和 python shell。要使用`pip`安装 Flask 脚本，请运行以下命令：
