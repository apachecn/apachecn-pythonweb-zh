...
我们已经按照[第 2 章](02.html#page "Chapter 2. Creating a Django Project")*创建 Django 项目*安装了 Python 和 PIP，但在安装 Django 之前，我们将安装virtualenv。此工具用于为 Python 创建虚拟环境，并在同一操作系统上具有不同的库版本。事实上，在使用 Debian 的许多 Linux系统上，已经安装了 Python 2 的一个版本。建议您不要卸载它以保持系统稳定。我们将安装 virtualenv 来设置我们自己的环境，并促进我们未来的 Django 迁移：
